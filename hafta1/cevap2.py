#soru 2: Kisinin adini ve soyadini alip Selamlama hitabi yaziniz.
# Yazarken ad ve soyad yer degistirsin ve aralarina 3 adet bosluk ekleyen program yaziniz.

#Kullanıcının Adını ve Soyadını istiyoruz.
adi=input("Lütfen Adınızı Giriniz...: ")
soyAdi=input("Lütfen Soyadınızı Giriniz...: ")

#Adı ve soyadını yazdırırken yer değiştiriyoruz ve aralarına 3 adet boşluk ekliyoruz.
boslukAdet=3 
print("Hoşgeldin",soyAdi,adi,sep=" "*boslukAdet)

